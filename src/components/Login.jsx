  ...
  return (
    <div className='fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex justify-center items-center'>
      ...
      <motion.form onSubmit={onSubmitHandler} initial={{opacity:0.2, y:50}} transition={{duration:0.3}} whileInView={{opacity: 1, y:0 }} viewport={{once: true}} className='relative bg-white p-10 rounded-xl text-slate-500'>
        ...
        {state !== 'Login' && <div className='border px-6 py-2 flex items-center gap-2 rounded-full mt-3'> 
        ...
        <button className='bg-blue-600 w-full text-white py-2 rounded-full cursor-pointer'>{state=='Login' ? 'login' : 'Create Accout'}</button>
